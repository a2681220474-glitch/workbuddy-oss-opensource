...
def main() -> int:
    parser = argparse.ArgumentParser(description="Inspect recent Feishu acceptance traces from WorkBuddy OSS.")
    ...
    diagnostics = fetch_json(f"{args.api_base.rstrip('/')}/api/channels/feishu/diagnostics/full", args.user)
    traces = diagnostics.get("acceptance_traces") if isinstance(diagnostics, dict) else []
    if not isinstance(traces, list):
        print("No acceptance traces returned from diagnostics endpoint.", file=sys.stderr)
        return 2

    filtered = [
        trace for trace in traces
        if trace_matches(trace, args.message_id, args.contains)
    ]
    if not filtered:
        print("No Feishu acceptance trace matched the requested filter.", file=sys.stderr)
        return 1
...
def render_report(summary: Any, trace: dict[str, Any]) -> str:
    ...
    for key, label in [
        ...
        ("routed", "routed"),
        ("business_object_created", "business_object_created"),
        ("approval_created", "approval_created"),
        ("timeline_ready", "timeline_ready"),
        ("send_completed", "send_completed"),
    ]:
        lines.append(f"  - {label}: {'yes' if checklist.get(key) else 'no'}")
    return "\n".join(lines)
